			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	#pragma omp parallel for schedule (guided) private (j,index_X,index_Y)
   	for(i = 0; i < n; i++){
...
void func2(double *weights, double *probability, int n) 
{
	...
	#pragma omp parallel for schedule (guided) reduction (+:sumWeights)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
	...
	#pragma omp parallel for schedule (guided) reduction (+:estimate_x,estimate_y)
	for(i = 0; i < n; i++){
		weight = weights[i];
